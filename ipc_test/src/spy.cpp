...
class ConsoleSpy {
public:
    ConsoleSpy(const std::string& json_file, const std::string& shm_name) 
    ...
    void run() {
        std::cout << "Console Memory Spy - Monitoring: " << shm_name_ << "\n";
...
private:
    ...
    void load_metadata() {
        ...
        if (metadata.contains("system")) {
            ...
            if (system.contains("architecture") && !system["architecture"].is_null()) {
                std::cout << "  Architecture: " << system["architecture"] << "\n";
            }
            if (system.contains("endianness") && !system["endianness"].is_null()) {
        ...
        if (metadata.contains("SimpleStruct")) {
            ...
            for (const auto& member : struct_info["members"]) {
                members_.push_back({
                    ...
                    member["offset"],
                    member["size"],
                    member.contains("type") ? member["type"].get<std::string>() : "unknown"
                });
            }
        }
    }
    ...
    void update() {
        ...
        if (shm_) {
            ...
            if (header->data_valid == 1) {
                ...
                display_data(data, std::string(header->struct_name));
            } else {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ConsoleSpy spy(json_file, test_name);
        spy.run();
    } catch (const std::exception& e) {
        std::cerr << "Error: " << e.what() << "\n";
        return 1;
